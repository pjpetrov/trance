...
def test_a_project_can_demand_a_check_after_every_step(tmp_path):
    """"Run the regressions after each step, catch the break early" as one
    project-wide rule, not a chip ticked onto every agent — which silently
    misses the next agent added. It lands at the end of each agent's own
    chain, and reaches agents that carry no checks at all."""
    from fastapi.testclient import TestClient
    ...
    client.put(f"/api/agents/frontend?session={sid}",
               json={"checks": ["factchecker", "reviewer"]})
    client.put(f"/api/config/planning?session={sid}",
               json={"always_check": ["regression"]})

    client.put(f"/api/sessions/{sid}/flow", json={"steps": [
        {"role": "frontend", "task": "build the lobby"},
        {"role": "planner", "task": "write the architecture"},
    ]})
    steps = client.get(f"/api/sessions/{sid}").json()["flow"]["steps"]

    # The agent's own order first, the project's floor at the end.
    assert steps[0]["checks"] == ["factchecker", "reviewer", "regression"]
    # And an agent with no checks of its own still gets the floor.
    assert steps[1]["checks"] == ["regression"]

    # A non-verifier is refused, with the reason.
    bad = client.put(f"/api/config/planning?session={sid}",
                     json={"always_check": ["frontend"]})
    assert bad.status_code == 400
    assert "not a verifier" in bad.json()["detail"]
