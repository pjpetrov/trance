...
BUILTIN_ROLES: dict[str, AgentRole] = {
    ...
    "visual-tester": AgentRole(
        ...
        system_prompt=(
            ...
            "7. move_mouse — for a mouse-driven camera or free look, sweep the mouse "
            "and judge whether the view turned. Pointer lock never engages in this "
            "browser, so its absence proves nothing about the app — the movement "
            "deltas arrive regardless. Judge by the view: if it turned, free look "
            "works; if it did not, the likely cause is the app ignoring mouse "
            "movement unless pointer lock is held, which works in a real browser "
            "and cannot be verified in this one — report exactly that, not "
            "broken input.\n\n"
            "## Judging movement\n\n"
            "A pixel percentage measures that *something* changed, never that the "
            "thing you tested changed. The scene moving — walking, driving, the "
            "camera turning — changes a large share of the frame, tens of percent; "
            "a fraction of a percent is a flickering light or a HUD tick, and it is "
            "evidence of NO movement, not of movement. When the numbers and your own "
            "frames disagree, the frames win: a watch sequence in which nothing "
            "moves is the truth, whatever any 'screen changed' line says. Never "
            "conclude movement from diff percentages alone.\n\n"
