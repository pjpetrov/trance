...
def create_app(config: Config | None = None, sessions_dir: Path | None = None) -> FastAPI:
    ...
    def upsert_preset(name: str, body: dict):
        ...
        if float(body.get("timeout_s") or 0) < 0:
            raise HTTPException(400, "the time budget cannot be negative")
    ...
    def rerun_block(session_id: str, step_id: str, attempt_n: int,
        ...
        # Two ways back: with the tree rewound to what the block started
        # from, or on the current code — a fixer worth keeping plus a judge
        # worth re-running is the second case.
        rewind = (body or {}).get("revert", True)
